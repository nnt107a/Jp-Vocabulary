...
function setupEventListeners() {
  ...
  document.querySelectorAll('.grammar-tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      } else if (targetTabId === 'tab-grammar-pdf') {
        const iframe = document.getElementById('grammar-pdf-iframe');
        if (iframe && !iframe.src) {
          iframe.src = iframe.dataset.src || 'public/Grammar/1.pdf';
        }
      }
    });
  });
  ...
  document.querySelectorAll('.grammar-mode-card').forEach(card => {
    card.addEventListener('click', () => {
      const mode = card.dataset.grammarMode;
      if (mode) {
        closeGrammarModal();
...
function renderGrammarParticlesTab() {
    ...
    const usagesHtml = p.usages.map(u => {
      const examplesHtml = u.examples.map(ex => `
        <div class="grammar-example-item">
          ...
          <button class="grammar-audio-btn" data-audio="${encodeURIComponent(ex.jp)}" title="Nghe phát âm">🔊</button>
        </div>
      `).join('');
  ...
  container.querySelectorAll('.grammar-audio-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const text = btn.dataset.audio;
      if (text) speakJapanese(decodeURIComponent(text));
    });
  });
}
    ...
    const combosHtml = item.combos.map(c => `
      <div class="grammar-usage-box" style="border-left-color: #c084fc;">
        ...
        <div class="grammar-example-item">
          ...
          <button class="grammar-audio-btn" data-audio="${encodeURIComponent(c.exampleQ)}" title="Nghe phát âm">🔊</button>
        </div>
      </div>
    `).join('');
