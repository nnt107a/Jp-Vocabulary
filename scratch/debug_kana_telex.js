...
function stripVietnameseAccents(str) {
  ...
  // 5. Tone mark mappings for circumflex vowels (e.g. tố -> tos, tồ -> tof, etc.)
...
function testConvert(input) {
  const clean = stripVietnameseAccents(input);
  const hiragana = wanakana.toHiragana(clean);
  console.log(`"${input}" -> stripped: "${clean}" -> hiragana: "${hiragana}"`);
}

console.log('--- CRITICAL TEST CASES FOR USER PROBLEM ---');
testConvert('おtô');
testConvert('おtôk');
testConvert('おtôko');
testConvert('tôko');
